...
transaction_report_types = models.Variable.get ('microspos_hourly_api_calls_getguestchecks', deserialize_json=True)
report_types = models.Variable.get ('microspos_hourly_api_calls', deserialize_json=True)

task_group_prefix = "LND_STG_HSTG_"
lndraw_prefix = "LND_RAW.LOAD_LND_" + source_system_code + "_"
lnd_prefix = "LND.LOAD_LND_HOURLY_" + source_system_code + "_"
stg_prefix = "STG.LOAD_STG_HOURLY_" + source_system_code + "_"
hstg_prefix = "HSTG.LOAD_HSTG_HOURLY_" + source_system_code + "_"
...
default_dag_args = {
    "owner": dag_owner,
    'start_date': datetime(2023, 10, 31, 15, tzinfo=local_tz),
...
with DAG ("microspos_hourly_data_load",
    #schedule_interval = "5 7-23,00-03 * * *",
    schedule_interval = "55 6-23,00-02 * * *", # updated on 2024-02-13 as per Luis
    ...
    data_ingestion_package = source_system_code + "_GETGUESTCHECKS" + '_HOURLY_DATA_INGESTION'
    data_load_package = source_system_code + "_GETGUESTCHECKS" + '_HOURLY_DATA_LOAD'

    with TaskGroup(group_id=data_ingestion_package) as getguestchecks_data_ingestion_process:
        task_group = 'GETGUESTCHECKS_HOURLY_API_REQUEST'
        ...
        for transaction_report_type in transaction_report_types:

            data_load_hourly_apis = GCPCloudFunctionOperator(
                task_id = lndraw_prefix + transaction_report_type,
                method='POST',
                http_conn_id='gcp_cf_dw_etl_microspos_1_hourly_api_to_bq',
                data={'api': transaction_report_type},
                endpoint='/',
                headers={},
            ...
            data_ingestion_module_start >> data_load_hourly_apis >> data_ingestion_module_end


    with TaskGroup(group_id=data_load_package) as getguestchecks_data_load_process:
        for transaction_report_type in transaction_report_types:
            transaction_report_type = transaction_report_type.upper()
            task_group = task_group_prefix + transaction_report_type
            lnd_table = lnd_prefix + transaction_report_type
            stg_table = stg_prefix + transaction_report_type
            hstg_table = hstg_prefix + transaction_report_type
        
            with TaskGroup (group_id=task_group) as LND_TO_HSTG:
    ...
    data_ingestion_package = source_system_code + '_HOURLY_DATA_INGESTION'
    data_load_package = source_system_code + '_HOURLY_DATA_LOAD'

    with TaskGroup(group_id=data_ingestion_package) as data_ingestion_process:
        task_group = 'HOURLY_API_REQUEST'
        data_ingestion_module_start = PythonOperator (
            task_id = "module_start",
            python_callable = dlf_pre_execute,
            op_kwargs = {
        ...
        for report_type in report_types:
            ...
            data_load_hourly_apis = GCPCloudFunctionOperator(
                task_id = lndraw_prefix + report_type,
                method='POST',
                http_conn_id='gcp_cf_dw_etl_microspos_1_hourly_api_to_bq',
...
data_load_batch_start >> getguestchecks_data_ingestion_process >> getguestchecks_data_load_process >> data_ingestion_process >> data_load_process >> data_load_batch_end
getguestchecks_data_ingestion_process >> data_load_batch_error
