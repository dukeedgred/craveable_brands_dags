...
report_types = models.Variable.get ('microspos_backfill_api_calls', deserialize_json=True)
discrepancy_report_types = models.Variable.get ('microspos_backfill_api_calls_discrepancies', deserialize_json=True)

task_group_prefix = "LND_STG_HSTG_"
lndraw_prefix = "LND_RAW.LOAD_LND_RAW_" + source_system_code + "_"
lnd_prefix = "LND.LOAD_LND_BACKFILL_" + source_system_code + "_"
stg_prefix = "STG.LOAD_STG_BACKFILL_" + source_system_code + "_"
hstg_prefix = "HSTG.LOAD_HSTG_BACKFILL_" + source_system_code + "_"
...
default_dag_args = {
    "owner": dag_owner,
    'start_date': datetime(2024, 9, 1, 6, tzinfo=local_tz),
...
with DAG ("microspos_backfill_data_load",
    ...
    with TaskGroup(group_id=data_ingestion_package) as data_ingestion_process:
        ...
        data_load_getOrderTypeDailyTotals = GCPCloudFunctionOperator(
            ...
            method='POST',
            http_conn_id='gcp_cf_dw_etl_microspos_backfill_0_api_to_bq',
            data={'api': 'getOrderTypeDailyTotals'},
            endpoint='/',
            headers={},
    ...
    with TaskGroup(group_id=data_ingestion_package) as data_discrepancy_backfill_process:
        ...
        data_load_getGuestChecks = GCPCloudFunctionOperator(
            ...
            method='POST',
            http_conn_id='gcp_cf_dw_etl_microspos_backfill_weekly_0_api_to_bq_discrepancy_guestchecks',
    ...
    with TaskGroup(group_id=data_load_package) as getguestchecks_data_load_process:
        for discrepancy_report in discrepancy_report_types:
            discrepancy_report = discrepancy_report.upper()
