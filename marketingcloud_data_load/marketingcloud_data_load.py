...
brand_types = models.Variable.get('marketingcloud_brands', deserialize_json=True)

source_system_code = "MARKETINGCLOUD"
dlf_batch_name = source_system_code + "_DATA_LOAD_BATCH"

report_types = models.Variable.get ('marketingcloud_api_calls', deserialize_json=True)
report_type_list = []
...
class GCPCloudFunctionOperator(HttpOperator):
    def execute(self, context):
        ...
        if self.data.get("brand") == 'RR' and self.data.get("api") == 'SENT_EVENT':
            for i in range(6):
                if response.text != "done." and i != 5:
                    self.log.info('Initial attempt failed. Retrying with retry data.')
                    ...
                    self.log.error('Response is not done. Marking task as failed.')
                    raise AirflowException('Response is not done.')
        else:
...
with DAG ("marketingcloud_data_load",
    ...
    with TaskGroup(group_id=data_load_package) as data_load_process:
        ...
        for brand_type in brand_types:
            for report_type in report_types:
                task_group = task_group_prefix + brand_type + "_" + report_type
                lnd_raw_table = lndraw_prefix + brand_type + "_" + report_type.upper()
                lnd_table = lnd_prefix + brand_type + "_" + report_type.upper()
                stg_table = stg_prefix + brand_type + "_" + report_type.upper()
                hstg_table = hstg_prefix + brand_type + "_" + report_type.upper()
                ...
                with TaskGroup (group_id=task_group) as STG_HSTG:
                    ...
                    data_load_lnd_raw = GCPCloudFunctionOperator(
                        ...
                        method='POST',
                        http_conn_id='dev_gcp_cf_dw_etl_marketingcloud_0_api_to_bq',
                        data={'api': report_type, 'brand': brand_type},
                        endpoint='/',
                        headers={},
                    ...
                    #data_load_module_start >> data_load_lnd_raw >> data_load_module_end
